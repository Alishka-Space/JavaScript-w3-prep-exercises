...
const findMentorForModule = (moduleName) => {
    ...
    // If no mentors can teach the module, return null
    if (mentorsForModule.length === 0) {
      return null;
    }
  
    // Randomly select a mentor from the filtered list
    const randomIndex = Math.floor(Math.random() * mentorsForModule.length);
    const randomMentor = mentorsForModule[randomIndex];
  
    // Return the name of the randomly selected mentor
    return randomMentor.name;
  };
  
  // Test the findMentorForModule function
